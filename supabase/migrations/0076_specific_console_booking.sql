-- 0076 Optional specific-unit online booking.
--
-- The capacity model (0038/0039) books a TYPE + time and assigns the unit at
-- check-in. Customers, though, have preferences (a specific billiard table, a
-- specific PS5). So we let a booking OPTIONALLY pin a console_id:
--   • get_venue_consoles — per-console busy intervals for a date (so the widget
--     can show which specific units are free in the picked window).
--   • create_marketplace_booking gains p_console_id (default null = "any", the
--     unchanged capacity behaviour). When set, we validate that exact unit is
--     free + belongs to the venue/type, and store it. The POOL capacity check
--     still runs, so the pool is never oversold and walk-ins are unaffected.

-- ── 1. per-console availability (anon) ───────────────────────────────────────
create or replace function public.get_venue_consoles(p_slug text, p_date date)
returns table(console_id int, name text, console_type text, busy jsonb)
...
grant execute on function public.get_venue_consoles(text, date) to anon, authenticated;

-- ── 2. create_marketplace_booking + optional p_console_id ────────────────────
drop function if exists public.create_marketplace_booking(
  text, timestamptz, integer, text, text, text, integer, integer, integer, text, text);
...
revoke execute on function public.create_marketplace_booking(
  text, timestamptz, integer, text, text, text, integer, integer, integer, text, text, integer) from public;
grant execute on function public.create_marketplace_booking(
  text, timestamptz, integer, text, text, text, integer, integer, integer, text, text, integer) to anon, authenticated;
