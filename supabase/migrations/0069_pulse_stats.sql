-- 0069 Martelounge Pulse — public, real-time Georgian gaming census.
--
-- A single anon-callable, SECURITY DEFINER function that returns ONLY aggregated,
-- non-personal stats across PUBLISHED, non-suspended venues. Powers the public
-- /live page (play.martelounge.ge/live). No customer/session PII is exposed —
-- only counts + each published venue's public profile + occupancy (same info the
-- marketplace already shows). Cross-tenant aggregation needs definer (bypass RLS).
...
begin
  ...
  with pub as (
    select v.id, v.name, v.city, v.lat, v.lng
  ...
  merged as (
    select p.id, p.name, p.city, p.lat, p.lng,
