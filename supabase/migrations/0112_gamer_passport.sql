-- 0112 - Gamer Passport engine (play.martelounge.ge): computed badges + XP/level from
-- EXISTING data (bookings, tournament registrations, wins, reviews). No new tables, no
-- triggers, no backfill -> always accurate, zero drift. The RPC returns badge KEYS +
-- progress (current/target); the Georgian titles/icons live in the frontend catalog so
-- this stays ASCII. A customer is marketplace_customers.id = auth.uid().
...
begin
  ...
  select count(*) filter (where status not in ('cancelled', 'no_show')),
         count(distinct venue_id) filter (where status not in ('cancelled', 'no_show')),
         coalesce(sum(total_amount) filter (where status not in ('cancelled', 'no_show')), 0)
